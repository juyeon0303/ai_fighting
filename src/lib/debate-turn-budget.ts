...
/** 중간에 끊긴 패턴 — COMPLETE_TAIL(…|다|…)보다 먼저 검사 */
const INCOMPLETE_TAIL =
  /(?:마다|에서|하여|이고|으며|라서|도록|처럼|같고|있고|없고|보면|때문|경우|것은|것이|있는|없는|되는|하는|이라|라는|으로|로서|위해|통해|대해|관해|따라|부터|마저|조차|밖에|뿐|만큼|정도|짓$|되$|하$|싶$|같$|줄$|올$|볼$|릴$|할$|될$|적$|법$|척$|하며$|이며$|라며$|면서$|면$|고$|서$|게$|듯$|듯한$|같은$|없$|있$|하는$|되는$|하려$|하려고$|하다$|이다$|한다$|된다$|겠$|였$|었$|까지(?:는|도)?|정도(?:까지)?|수준(?:까지)?|하려(?:고)?|할\s*수\s*있(?:는|을)?|것(?:은|이)?|하다\s*가)\s*다\s*$/;
